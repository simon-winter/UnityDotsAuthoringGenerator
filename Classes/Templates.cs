...
internal class Templates {
    ...
    {
        if (path == "") {
            path = Utils.AskUserForPath("Provide a location for your template files. Note: All files in and below given folder" + " will be made available as pastable template!", "", SettingsManager.FILES_PATH);
        }
        if (path != "") {
    ...
    {
        if (path == "") {
            path = Utils.AskUserForPath("Provide a location for your template snippets. Note: All files in and below given folder" + " will be processed and made be available as pastable snippets!", "", SettingsManager.SNIPPETS_PATH);
        }
        if (path != "") {
    ...
    private List<(string name, string content)> loadFiles(string path)
    {
        ...
        try {
            ...
            MessageBox.Show(string.Format("Failed reading directory: {0}", ex.Message), "Failed reading directory");
            return fileContents;
        }
