...
public partial class TemplateWindowControl : UserControl {
    ...
    public TemplateWindowControl()
    {
        ...
        this.Loaded += (object sender, RoutedEventArgs e) =>
        {
            ...
            if (filesPath == "" && snippetsPath == "") {
                var result = MessageBox.Show("No template paths in settings configured.\nDo you want to generate the default installation?\n\n" + "This will generate a folder with some data in the root of the project and set all links in settings accordingly.",
                    "Default installation?", MessageBoxButton.OKCancel, MessageBoxImage.Question);
                if (result == MessageBoxResult.OK) {
                    m_templates.GenerateDefaultInstallation();
            ...
            if (filesPath != "") {
                foreach (var file in m_templates.LoadFiles(filesPath)) {
                    wrapPanel_Files.Children.Add(new Button() {
                        ...
                        Command = new CreateFile(Path.GetExtension(file.name), file.content)
                    });
                };
            }
